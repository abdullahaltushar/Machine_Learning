...
data=saledata[['TRANSDT','AMOUNT','QTY','RATE']]
...
history = model.fit(train1X, train1Y, epochs=1000,batch_size=64,validation_split=0.1, verbose=1)
...
df_forecast1 = pd.DataFrame({'TRANSDT':np.array(forecast_dates), 'AMOUNT':y_pred_future1})
df_forecast1['TRANSDT']=pd.to_datetime(df_forecast1['TRANSDT'])
...
sns.lineplot(df_forecast1['TRANSDT'], df_forecast1['AMOUNT'])